...
public:
    ...
    void MainLoop()
    {
        ...
        ALOGI("VR Extra Performance Mode: {}",
              VRSettings::values.extra_performance_mode_enabled ? "enabled"
                                                                : "disabled");
        ...
        mInputStateStatic = std::make_unique<InputStateStatic>(
            OpenXr::GetInstance(), gOpenXr->mSession);
        ...
        // Create the game surface layer.
        {
            ...
            const uint32_t resolutionFactorFromPreferences =
            ...
            const uint32_t immersiveModeOffset =
                (VRSettings::values.vr_immersive_mode > 0) ? 2 : 0;
            const uint32_t resolutionFactor =
                (resolutionFactorFromPreferences > 0
            ...
            mGameSurfaceLayer = std::make_unique<GameSurfaceLayer>(
                XrVector3f{0, 0, -2.0f}, jni, mActivityObject,
                gOpenXr->mSession, resolutionFactor);
        }
...
private:
    void Frame(JNIEnv* jni)
    {
        ...
        mInputStateFrame.SyncButtonsAndThumbSticks(gOpenXr->mSession,
                                                   mInputStateStatic);
...

        {
            ...
            const auto& leftThumbrestTouchState =
                mInputStateFrame
                    .mThumbrestTouchState[InputStateFrame::LEFT_CONTROLLER];
            const auto& rightThumbrestTouchState =
                mInputStateFrame
                    .mThumbrestTouchState[InputStateFrame::RIGHT_CONTROLLER];
        ...
        if (mFrameIndex == 1)
        {
            ...
            OXR(xrLocateSpace(gOpenXr->mForwardDirectionSpace,
                              gOpenXr->mLocalSpace,
        ...
        mInputStateFrame.SyncHandPoses(gOpenXr->mSession, mInputStateStatic,
                                       gOpenXr->mLocalSpace,
        ...
        std::vector<XrCompositionLayer> layers(gOpenXr->mMaxLayerCount,
                                               XrCompositionLayer{});
    ...
    void HandleSessionStateChanges(const XrSessionState state)
    {
        if (state == XR_SESSION_STATE_READY)
        {
            ...
            sbi.primaryViewConfigurationType =
                gOpenXr->mViewportConfig.viewConfigurationType;
            ...
            if (mIsXrSessionActive)
            {
                ...
                OXR(pfnPerfSettingsSetPerformanceLevelEXT(
                    gOpenXr->mSession, XR_PERF_SETTINGS_DOMAIN_GPU_EXT,
                    kGpuPerfLevel));
