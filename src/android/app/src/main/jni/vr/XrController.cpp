...
InputStateStatic::InputStateStatic(const XrInstance& instance,
    ...
    mThumbRestTouchAction =
        CreateAction(mActionSet, XR_ACTION_TYPE_BOOLEAN_INPUT, "thumbresttouch",
                     nullptr, 2, handSubactionPaths);
    ...
    // Create bindings for Quest controllers.
    {
        ...
        bindings.push_back(
            ActionSuggestedBinding(instance, mThumbRestTouchAction,
...
SyncButtonState(const XrSession& session,
...
void InputStateFrame::SyncButtonsAndThumbSticks(
    ...
    mThumbrestTouchState[LEFT_CONTROLLER] =
        SyncButtonState(session, staticState->mThumbRestTouchAction,
                        staticState->mLeftHandSubactionPath);
    mThumbrestTouchState[RIGHT_CONTROLLER] =
        SyncButtonState(session, staticState->mThumbRestTouchAction,
                        staticState->mRightHandSubactionPath);
