...
private : void
    ...
    mRibbonLayer = std::make_unique<RibbonLayer>(
        XrVector3f{0, -0.75f, -1.51f}, XrMath::Quatf::FromEuler(-MATH_FLOAT_PI / 4.0f, 0.0f, 0.0f),
    ...
    if (mForwardVRJoystickMethodID == nullptr) { FAIL("could not get forwardVRJoystickMethodID"); }

    mSendClickToWindowMethodID =
        jni->GetMethodID(jni->GetObjectClass(mActivityObject), "sendClickToWindow", "(FFI)V");
    if (mSendClickToWindowMethodID == nullptr) { FAIL("could not get sendClickToWindowMethodID"); }
    mResumeGameMethodID =
    ...
    mPauseGameMethodID = jni->GetMethodID(jni->GetObjectClass(mActivityObject), "pauseGame", "()V");
    ...
    if (VRSettings::values.vr_immersive_mode != 0) { mLastAppState.mIsLowerMenuToggledOn = false; }
}
...
void Frame(JNIEnv* jni, const AppState& appState) {
    ...
    if (mFrameIndex == 1) { CreateRuntimeInitatedReferenceSpaces(frameState.predictedDisplayTime); }
    ...
    const bool isImmersiveModeEnabled = [this](const bool showUIRibbon, float& immersiveModeFactor) -> bool {
...
void HandleInput(JNIEnv* jni, const InputStateFrame& inputState, AppState& newState) const {
    ...
    ForwardButtonStateIfNeeded(jni, mActivityObject, mForwardVRInputMethodID, 100 /* BUTTON_Y */,
                               inputState.mYButtonState, "y");
    ForwardButtonStateIfNeeded(jni, mActivityObject, mForwardVRInputMethodID, 102 /* BUTTON_L1 */,
                               inputState.mSqueezeTriggerState[InputStateFrame::LEFT_CONTROLLER],
                               "l1");
    ForwardButtonStateIfNeeded(jni, mActivityObject, mForwardVRInputMethodID, 103 /* BUTTON_R1 */,
                               inputState.mSqueezeTriggerState[InputStateFrame::RIGHT_CONTROLLER],
                               "r1");

    {
...

        {
            ...
            if (hasDpad && dpadThumbstickState.currentState.y > kThumbStickDirectionThreshold) {
                jni->CallVoidMethod(mActivityObject, mForwardVRInputMethodID, 19 /* DPAD_UP */, 1);
                wasDpadUp = true;
            } else if (wasDpadUp) {
                jni->CallVoidMethod(mActivityObject, mForwardVRInputMethodID, 19 /* DPAD_UP */, 0);
                wasDpadUp = false;
            }
            if (hasDpad && dpadThumbstickState.currentState.y < -kThumbStickDirectionThreshold) {
                jni->CallVoidMethod(mActivityObject, mForwardVRInputMethodID, 20 /* DPAD_DOWN */,
                                    1);
                wasDpadDown = true;
            } else if (wasDpadDown) {
                jni->CallVoidMethod(mActivityObject, mForwardVRInputMethodID, 20 /* DPAD_DOWN */,
            ...
            if (hasDpad && dpadThumbstickState.currentState.x < -kThumbStickDirectionThreshold) {
                jni->CallVoidMethod(mActivityObject, mForwardVRInputMethodID, 21 /* DPAD_LEFT */,
                                    1);
                wasDpadLeft = true;
            } else if (wasDpadLeft) {
                jni->CallVoidMethod(mActivityObject, mForwardVRInputMethodID, 21 /* DPAD_LEFT */,
            ...
            if (hasDpad && dpadThumbstickState.currentState.x > kThumbStickDirectionThreshold) {
                jni->CallVoidMethod(mActivityObject, mForwardVRInputMethodID, 22 /* DPAD_RIGHT */,
                                    1);
                wasDpadRight = true;
...
void HandleCursorLayer(JNIEnv* jni, const AppState& appState, const bool showLowerPanel,
    ...
    CursorLayer::CursorType cursorType = appState.mLowerMenuType == LowerMenuType::POSITIONAL_MENU
           ...
           !mInputStateFrame.mIsHandActive[nonPreferredController]);
    {
        ...
        sIsLowerPanelBeingPositioned &= appState.mLowerMenuType == LowerMenuType::POSITIONAL_MENU &&
        ...
        if (isPreferredControllerActive) {
            ...
            if (!shouldRenderCursor && appState.mLowerMenuType == LowerMenuType::POSITIONAL_MENU) {
                ...
                if (shouldRenderCursor && triggerState.currentState) {
                    ...
                    static constexpr float kThumbStickDirectionThreshold = 0.5f;
                    if (std::abs(thumbstickState.currentState.y) > kThumbStickDirectionThreshold) {
            ...
            if (!shouldRenderCursor) {
                ...
                ForwardButtonStateIfNeeded(
                    ...
                    mInputStateFrame.mIndexTriggerState[InputStateFrame::LEFT_CONTROLLER], "l2");
                ForwardButtonStateIfNeeded(
                    jni, mActivityObject, mForwardVRInputMethodID, 105 /* BUTTON_R2 */,
                    mInputStateFrame.mIndexTriggerState[InputStateFrame::RIGHT_CONTROLLER], "r2");
            }
    ...
    if (shouldRenderCursor) {
        ...
        mCursorLayer->Frame(gOpenXr->mLocalSpace, quadLayer, cursorPose3d, scaleFactor, cursorType);
...
void OXRHandleSessionStateChangedEvent(JNIEnv*                               jni,
    ...
    if (newState.state != lastState) {
        ALOGV("{}(): Received XR_SESSION_STATE_CHANGED state {}->{} "
              "session={} time={}",
              __func__, XrSessionStateToString(lastState), XrSessionStateToString(newState.state),
              (void*)newState.session, newState.time);
    }
    ...
    if (state == XR_SESSION_STATE_READY) {
        ...
        if (newAppState.mIsXrSessionActive) {
            ...
            OXR(pfnPerfSettingsSetPerformanceLevelEXT(
                gOpenXr->mSession, XR_PERF_SETTINGS_DOMAIN_CPU_EXT, VRSettings::values.cpu_level));
            OXR(pfnPerfSettingsSetPerformanceLevelEXT(
            ...
            PFN_xrSetAndroidApplicationThreadKHR pfnSetAndroidApplicationThreadKHR = NULL;
            OXR(xrGetInstanceProcAddr(gOpenXr->mInstance, "xrSetAndroidApplicationThreadKHR",
                                      (PFN_xrVoidFunction*)(&pfnSetAndroidApplicationThreadKHR)));

            if (vr::gPriorityTid > 0) {
                ALOGD("Setting prio tid from main {}", vr::gPriorityTid);
                OXR(pfnSetAndroidApplicationThreadKHR(
                    gOpenXr->mSession, XR_ANDROID_THREAD_TYPE_RENDERER_MAIN_KHR, vr::gPriorityTid));
            } else {
        ...
        switch (message.mType) {
            ...
            case Message::Type::CHANGE_LOWER_MENU: {
                ...
                ALOGD("Received CHANGE_LOWER_MENU message: {}, state change {} -> {}",
                      message.mPayload, mLastAppState.mLowerMenuType, newAppState.mLowerMenuType);
                break;
            }
...
void UpdateImmersiveModeIfNeeded(const float immersiveModeFactor) const {
    ...
    const float gamePosScaler = powf(10.f, VRSettings::values.vr_immersive_positional_game_scaler) *
