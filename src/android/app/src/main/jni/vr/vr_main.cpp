...
namespace
{
constexpr XrPerfSettingsLevelEXT kCpuPerfLevel =
                                ...
                                const char*                 buttonName)
{
    ...
    {
        ALOG_INPUT_VERBOSE("Forwarding %s button state: %d", buttonName,
                           actionState.currentState);
        ForwardButtonStateChangeToCitra(
            jni, activityObject, forwardVRInputMethodID, androidButtonCode,
            actionState.currentState);
    }
}
...
private:
    void Frame(JNIEnv* jni)
    {
        ...
        if (mFrameIndex == 1)
        {
            ...
            ALOGI("Time to first frame: %lld ms",
                  std::chrono::duration_cast<std::chrono::milliseconds>(
                      now - gOnCreateStartTime)
                      .count());
        }

        mInputStateFrame.SyncButtonsAndThumbSticks(gOpenXr->session_,
                                                   mInputStateStatic);
        ...
        ForwardButtonStateIfNeeded(jni, mActivityObject,
                                   mForwardVRInputMethodID, 96 /* BUTTON_A */,
                                   mInputStateFrame.mAButtonState, "a");
        ForwardButtonStateIfNeeded(jni, mActivityObject,
                                   mForwardVRInputMethodID, 97 /* BUTTON_B */,
                                   mInputStateFrame.mBButtonState, "b");
        ForwardButtonStateIfNeeded(jni, mActivityObject,
                                   mForwardVRInputMethodID, 99 /* BUTTON_X */,
                                   mInputStateFrame.mXButtonState, "x");
        ForwardButtonStateIfNeeded(jni, mActivityObject,
                                   mForwardVRInputMethodID, 100 /* BUTTON_Y */,
                                   mInputStateFrame.mYButtonState, "y");
        ForwardButtonStateIfNeeded(
            jni, mActivityObject, mForwardVRInputMethodID, 102 /* BUTTON_L1 */,
            mInputStateFrame
                .mSqueezeTriggerState[InputStateFrame::LEFT_CONTROLLER],
            "l1");
        ForwardButtonStateIfNeeded(
            jni, mActivityObject, mForwardVRInputMethodID, 103 /* BUTTON_R1 */,
            mInputStateFrame
                .mSqueezeTriggerState[InputStateFrame::RIGHT_CONTROLLER],
            "r1");

        {
            ...
            const auto& leftThumbstickClickState =
                mInputStateFrame
                    .mThumbStickClickState[InputStateFrame::LEFT_CONTROLLER];
            const auto& rightThumbstickClickState =
                mInputStateFrame
                    .mThumbStickClickState[InputStateFrame::RIGHT_CONTROLLER];
            const int dpadHand = leftThumbstickClickState.currentState
                                     ? InputStateFrame::LEFT_CONTROLLER
                                 : rightThumbstickClickState.currentState
                                     ...
                                     : InputStateFrame::NUM_CONTROLLERS;
            {
                ...
                const bool hasDpad =
                    dpadHand != InputStateFrame::NUM_CONTROLLERS;
        ...
        if (mFrameIndex == 1)
        {
            ...
            // starting frame.
            {
                const XrReferenceSpaceCreateInfo sci = {
                    XR_TYPE_REFERENCE_SPACE_CREATE_INFO, nullptr,
                    XR_REFERENCE_SPACE_TYPE_LOCAL, XrMath::Posef::Identity()};
            ...
            OXR(xrLocateSpace(gOpenXr->forwardDirectionSpace_,
                              gOpenXr->localSpace_,
        ...
        mInputStateFrame.SyncHandPoses(gOpenXr->session_, mInputStateStatic,
                                       gOpenXr->localSpace_,
        ...
        std::vector<XrCompositionLayer> layers(gOpenXr->maxLayerCount_,
                                               XrCompositionLayer{});
        ...
        {
            {
                ...
                [[maybe_unused]] const auto nonPreferredController =
                    mInputStateFrame.mPreferredHand ==
                            InputStateFrame::LEFT_CONTROLLER
                        ? InputStateFrame::RIGHT_CONTROLLER
                        : InputStateFrame::LEFT_CONTROLLER;
...

                {
                    ...
                    if (isPreferredControllerActive)
                    {
                        ...
                        const auto& triggerState =
                            mInputStateFrame.mIndexTriggerState
                                [mInputStateFrame.mPreferredHand];
                        ...
                        if (gShouldShowErrorMessage)
                        {
                            ...
                            if (triggerState.changedSinceLastSync)
                            {
                                mErrorMessageLayer->SendClickToWindow(
                                    pos2d, triggerState.currentState);
                            }
#endif
                        }
                        else
                        {
                            ...
                            ALOG_INPUT_VERBOSE("Cursor 2D coords: %f %f",
                                               pos2d.x, pos2d.y);
                        ...
                        if (!shouldRenderCursor)
                        {
                            ...
                            if (shouldRenderCursor && triggerState.currentState)
                            {
                                ...
                                mGameSurfaceLayer->SetTopPanelFromController(
                                    XrVector3f{0, cursorPose3d.position.y,
                                               cursorPose3d.position.z});
                                // If trigger is pressed, thumbstick controls
                                // the depth
                                const XrActionStateVector2f& thumbstickState =
                                    mInputStateFrame.mThumbStickState
                                        [mInputStateFrame.mPreferredHand];
                        ...
                        {
                            const float distance = XrMath::Vector3f::Length(
                                pose.position - cursorPose3d.position);
                ...
                {
                    XrCompositionLayerQuad quadLayer = {};
                    mCursorLayer->Frame(gOpenXr->localSpace_, quadLayer,
    ...
    void HandleSessionStateChanges(const XrSessionState state)
    {
        if (state == XR_SESSION_STATE_READY)
        {
            ...
            sbi.primaryViewConfigurationType =
                gOpenXr->viewportConfig_.viewConfigurationType;
            ...
            if (mIsXrSessionActive)
            {
                ...
                OXR(pfnPerfSettingsSetPerformanceLevelEXT(
                    gOpenXr->session_, XR_PERF_SETTINGS_DOMAIN_CPU_EXT,
                    kCpuPerfLevel));
                OXR(pfnPerfSettingsSetPerformanceLevelEXT(
                    gOpenXr->session_, XR_PERF_SETTINGS_DOMAIN_GPU_EXT,
                    kGpuPerfLevel));

                PFN_xrSetAndroidApplicationThreadKHR
                    pfnSetAndroidApplicationThreadKHR = NULL;
                OXR(xrGetInstanceProcAddr(
        ...
        const XrEventDataSessionStateChanged& newState)
    {
        ...
        if (newState.state != lastState)
        {
            ALOGV("%s(): Received XR_SESSION_STATE_CHANGED state %s->%s "
                  ...
                  XrSessionStateToString(newState.state), newState.session,
                  newState.time);
        }
        ...
        {
            case XR_SESSION_STATE_FOCUSED:
                ALOGV("%s(): Received XR_SESSION_STATE_FOCUSED event",
                      __func__);
                if (!mHasFocus)
                {
                ...
                break;
            case XR_SESSION_STATE_VISIBLE:
                ALOGV("%s(): Received XR_SESSION_STATE_VISIBLE event",
                      __func__);
                if (mHasFocus)
                {
